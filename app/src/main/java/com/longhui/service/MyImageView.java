...
public class MyImageView extends ImageView {
	...
	protected void onDraw(Canvas canvas) {
		...
		if (width > PshowActivity.screenWidth) {
		// 如果图宽大于屏宽，就不用水平居中
		center(false, true);
		} else {
		center(true, true);
	...
	private void arithScaleRate() {
		float scaleWidth = PshowActivity.screenWidth / (float) imageWidth;
		float scaleHeight = PshowActivity.screenHeight / (float) imageHeight;
	...
	public void setImageBitmap(Bitmap bitmap) {
		...
		arithScaleRate();
		//缩放到屏幕大小
		zoomTo(scaleRate,PshowActivity.screenWidth / 2f, PshowActivity.screenHeight / 2f);
...
//		RectF rect = new RectF(0, 0, imageWidth*getScale(), imageHeight*getScale());
	...
	public void layoutToCenter()
	{
		...
		//空白区域宽高
		float fill_width = PshowActivity.screenWidth - width;
		float fill_height = PshowActivity.screenHeight - height;
	...
	protected float getValue(Matrix matrix, int whichValue) {
		...
		mMinZoom =( PshowActivity.screenWidth/2f)/imageWidth;
